    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Obx(() => _buildFeaturedProjects(controller.filteredProjects.where((p) => p.isFeatured).toList())),
          const SizedBox(height: 60),
          Obx(() => _buildProjectGrid(context, controller.filteredProjects.where((p) => !p.isFeatured).toList())),
        ],
      ),
    );
  }
        ...
        return Obx(() {
          ...
          return ChoiceChip(
            ...
            shape: RoundedRectangleBorder(
              ...
              side: BorderSide(color: isSelected ? AppColors.gold : (Theme.of(context).dividerTheme.color ?? AppColors.darkBorder)),
            ),
          );
        });
      }).toList(),
...
class _FeaturedProjectCard extends StatelessWidget {
  ...
  Widget _buildImage(BuildContext context) {
    ...
    return Container(
      ...
      child: Center(
        child: Icon(AppIcons.laptop, size: isMobile ? 60 : 100, color: AppColors.gold.withValues(alpha: 0.1)),
      ),
    );
  }
      ...
      child: Column(
        ...
        children: [
          const Text(
            ...
            style: TextStyle(color: AppColors.gold, letterSpacing: 2, fontSize: 12),
          ),
          const SizedBox(height: 20),
          Text(
            project.name,
            textAlign: isMobile ? TextAlign.center : TextAlign.start,
            style: TextStyle(fontSize: 32, fontWeight: FontWeight.bold, color: Theme.of(context).textTheme.displayLarge?.color),
          ),
          const SizedBox(height: 20),
          Text(
            project.description,
            textAlign: isMobile ? TextAlign.center : TextAlign.start,
            style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant, fontSize: 16, height: 1.6),
          ),
          ...
          Wrap(
            ...
            children: project.technologies.map((tech) => Text(
              "#$tech",
              style: const TextStyle(color: AppColors.goldAccent, fontSize: 14),
            )).toList(),
          ),
          const SizedBox(height: 40),
          Row(
            ...
            children: [
              ...
              TextButton(
                onPressed: () {},
                child: const Text(AppStrings.viewCaseStudy, style: TextStyle(color: AppColors.gold, fontWeight: FontWeight.bold)),
              ),
            ],
          ),
        ],
...
class _ProjectGridCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final hoverController = Get.put(ProjectHoverController(), tag: project.name);
    ...
    return MouseRegion(
      ...
      child: Obx(() => AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: hoverController.isHovered.value ? AppColors.gold : (Theme.of(context).dividerTheme.color ?? Colors.grey),
            width: 0.5,
          ),
          ...
          children: [
            Expanded(
              child: Container(
                color: Theme.of(context).colorScheme.surface,
                width: double.infinity,
                child: Icon(AppIcons.image, size: 50, color: AppColors.gold.withValues(alpha: 0.1)),
              ),
            ),
            Padding(
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      const Icon(AppIcons.arrowForward, size: 16, color: AppColors.gold),
                    ],
                  ),
                ],
              ),
